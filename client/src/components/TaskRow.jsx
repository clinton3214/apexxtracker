...
export default function TaskRow({ task, goal, onToggle, onUpdateSpent, onPullToToday, onDelete }) {
  ...
  return (
    <div className={`glass-panel group p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 transition-colors relative ${task.status === 'done' ? 'opacity-65' : 'hover:border-surface-borderStrong/60 hover:bg-surface/60'}`}>
      {/* Bin icon — top right, fades in on hover */}
      {onDelete && (
        <button
          onClick={() => onDelete(task.id)}
          title="Delete task"
          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-zinc-600 hover:text-rose-400 hover:bg-rose-500/10 rounded-md p-1"
        >
          <BinIcon />
        </button>
      )}
